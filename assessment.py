...
#         output dictionary.
#         '''
    s = string.lower() #so you don't have to worry about upper and lower cases
    letters = set(set(s) | set(s)) # get the unique set of input
    return({letters:s.count(letters) for letters in letters}) 
...
def array_work(rows, cols, scalar, matrixA):
        ...
        E.g., array_work(2, 3, 5, [[3, 4], [5, 6], [7, 8]])
               [[3, 4],      [[5, 5, 5],
                ...
                [7, 8]]
        '''
        matrixB = np.full((rows, cols), scalar)
        result1 = np.matmul(matrixA, matrixB )
        return result1
...
def make_series(start, length, index):
        ...
        dtype: int64
        '''
        return( pd.Series(range(start, start+length), index=list(index)))
